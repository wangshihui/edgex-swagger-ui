...
const (
	DefaultVersion            = "v2"
	DeviceSdkTemplateFileName = "device-sdk.yaml"
	CoreDataFileName          = "core-data.yaml"
	CoreMetaFileName          = "core-metadata.yaml"
	CoreCmdFileName           = "core-command.yaml"
	NotificationFileName      = "support-notifications.yaml"
	SchedulerFileName         = "support-scheduler.yaml"
	SysAgentFileName          = "system-agent.yaml"
)

type SwaggerApiKeyAuth struct {
	Type string `json:"type",yml:"type"`
	In   string `json:"in",yml:"in"`
	Name string `json:"name",yml:"name"`
}

type SwaggerSecurityComponets struct {
...
// GetSwaggerComponents returns the service's InsecureSecrets.
func (c *ConfigurationStruct) GetSwaggerComponents() SwaggerConfigComponents {
	return c.Swagger
}
